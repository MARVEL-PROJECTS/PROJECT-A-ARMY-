...
public class PlayerMovement : MonoBehaviour, EnemyHandler.IEnemyTargetable {
    ...
    [SerializeField] private Material playerAnimatedOutlineMaterial;
    ...
    private void OnTriggerEnter2D(Collider2D collider) {
        if (collider.name == "Star") {
            // Player Grabbed Star
            Destroy(collider.gameObject);
            transform.Find("Body").GetComponent<MeshRenderer>().material = playerAnimatedOutlineMaterial;
        }

        if (collider.name == "Door") {
            //collider.gameObject.GetComponent<DoorAnims>().PlayOpenFailAnim();
        }
    }
